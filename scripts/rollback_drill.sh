...
if [ "$DRY_RUN" = "1" ]; then
  echo "  [dry-run] vercel rollback https://kendrew.ai"
else
  read -r -p "Execute Vercel rollback? (y/N) " ans
  [ "$ans" = "y" ] && vercel rollback https://kendrew.ai
fi
echo
echo "Step 4: Verify /health green"
if [ "$DRY_RUN" = "0" ]; then
  for i in 1 2 3 4 5; do
    code=$(curl -s -o /dev/null -w "%{http_code}" https://app.kendrew.ai/health || echo 000)
    if [ "$code" = "200" ]; then
