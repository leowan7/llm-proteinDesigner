...
| B | curl + Sentry dashboard + Slack | `curl -i https://app-staging.bindwave.com/debug/sentry-test` (expect 500); confirm Sentry -> kendrew-backend -> Issues shows `ZeroDivisionError` within 1 min; confirm #kendrew-alerts receives Sentry post within 2 min; launch a draft job on staging and confirm Sentry Performance shows a `POST /jobs/launch` transaction with `traces_sample_rate=1.0`. | ~10 min |
| C | uptimerobot.com | Create HTTPS monitors for `https://app.bindwave.com/health` and `https://app-staging.bindwave.com/health` at 5-min intervals; add Slack webhook + email alert contacts; wait 10 min; confirm both show "Up". | ~10 min |
