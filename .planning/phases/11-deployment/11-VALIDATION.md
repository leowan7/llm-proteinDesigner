...
| 3 | Supabase Cloud (Free tier) with pooling + daily backup (7-day retention) | `/health` returns `db: {status: ok, pool: connected}`; daily backup retention visible in Supabase dashboard. PITR validation deferred until Pro upgrade before public launch. | `curl -fs https://app.kendrew.ai/health \| jq .db.status` equals `ok` | `/health` endpoint + manual backup-config verification |
