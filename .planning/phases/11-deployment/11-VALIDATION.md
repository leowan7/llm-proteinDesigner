...
|----|-------------------|-------------------|-----------------|----------------|
| 1 | Frontend on Vercel with custom domain + SSL | `curl -sI https://bindwave.com` returns 200 with `strict-transport-security` and Vercel server header | `curl -fsSI https://bindwave.com \| grep -i 'strict-transport'` | Manual rollout checklist + smoke.yml assertion |
| 2 | Backend + worker on Railway, Docker, auto-deploy from main | `curl https://app.bindwave.com/health` returns 200 after a push to main | `gh run list --workflow test.yml --limit 1 --json conclusion -q '.[0].conclusion'` returns `success`, then `curl -fs https://app.bindwave.com/health` | CI `test.yml` + Railway deploy status + smoke.yml |
| 3 | Supabase Cloud (Free tier) with pooling + daily backup (7-day retention) | `/health` returns `db: {status: ok, pool: connected}`; daily backup retention visible in Supabase dashboard. PITR validation deferred until Pro upgrade before public launch. | `curl -fs https://app.bindwave.com/health \| jq .db.status` equals `ok` | `/health` endpoint + manual backup-config verification |
| 4 | Upstash Redis with TLS | `/health` returns `redis: {status: ok, tls: true}`; `REDIS_URL` uses `rediss://` | `curl -fs https://app.bindwave.com/health \| jq .redis` | `/health` endpoint |
...
|----------|----|------------|-------------------|
| DNS records resolve correctly at Cloudflare apex (orange) + `app.*` grey-cloud | 1, 2 | DNS propagation is wall-clock dependent; cannot run in CI reliably | `dig bindwave.com`, `dig app.bindwave.com`, confirm CNAME targets and cloud flag expectations; verify LetsEncrypt cert issues for `app.bindwave.com` (grey-cloud required) |
| Resend domain verification (SPF/DKIM/DMARC) | 7, 8 | TXT record propagation + Resend API verify button | Follow `docs/deploy.md` Resend section; confirm Resend dashboard shows "Verified" for `bindwave.com` |
