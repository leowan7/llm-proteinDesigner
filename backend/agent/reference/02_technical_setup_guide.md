...
3. Filter: ipTM >= 0.70, pLDDT >= 80, i_pAE <= 10 A

The RFdiffusion container applies these itself, on **raw scales** (pLDDT 0–100, pAE in A):
`IPTM_THRESHOLD = 0.70`, `PLDDT_THRESHOLD = 80.0`, `IPAE_THRESHOLD = 10.0`
(`docker/rfdiffusion/run_pipeline.py:68-70`), ANDed into each design's `filter_status` at
`:515-517`. Only those three legs are enforced — there is **no** buried-SASA cutoff in the
code, so do not quote one as a platform reject rule.

None of these are BindCraft's filters and must not be quoted as such: BindCraft ships its own
`default_filters.json` on normalized 0–1 scales with different values. See §3,
...
`lengths` is **not** user-configurable here. `binder_length` is not a BindCraft wizard
parameter (`WIZARD_PARAMS["bindcraft"]` is `num_designs`, `design_cycles`,
`mpnn_sampling_temp`, `filter_score_threshold`), so a requested length is dropped before a
job row is created and `generate_config` falls through to its `[50, 100]` default
(`backend/pipelines/bindcraft.py:135-137`). Every customer run is a 50–100 aa miniprotein.
`design_path` and `binder_name` are likewise set by the pipeline, not the user.

### Design Protocol and Filter Set

Both are **hardwired**. Neither is a user-selectable parameter, so never offer a choice.

| Setting | Value every customer run uses |
|---|---|
| Advanced settings ("protocol") | `default_4stage_multimer.json` |
| Filter set | `default_filters.json` |

`backend/pipelines/bindcraft.py:156-157` returns both as literals, and neither `protocol` nor
`filter_set` is a BindCraft wizard parameter, so a user override is dropped before the job is
created. Upstream FreeBindCraft also ships `beta_sheet_4stage_multimer.json`,
`peptide_4stage_multimer.json`, `relaxed_filters.json` and `peptide_filters.json`; none of
those strings appears anywhere in this repo's code. Do not offer them. In particular **do not
offer BindCraft for peptides** — see *What It Designs Best* in `01_tool_selection_guide.md`.

(The internal `smoke` tier sends `filter_set: "no_filters.json"`, honoured by
`_resolve_filter_set` at `docker/bindcraft/run_pipeline.py:749`. That is a plumbing test,
never a customer run.)
