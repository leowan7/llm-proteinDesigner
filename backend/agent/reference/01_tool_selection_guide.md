...
**ipSAE (BindCraft-specific):** Alternative interface metric using per-residue d0 normalization based on interface contact count. Range 0–1; higher is better. Reported alongside ipTM; there is no rank-by switch on this platform.
...
### BindCraft
- Zero designs passing all filter stages → Expected on hard targets. There is no looser filter set to switch to; `default_filters.json` is hardwired (see *Design Protocol and Filter Set* in `02_technical_setup_guide.md`). Widen the hotspot selection, pick a more concave epitope, or switch tool
